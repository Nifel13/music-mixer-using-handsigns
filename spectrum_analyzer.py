...
class SimpleSpectrumAnalyzer:
    """Simple spectrum analyzer that just returns spectrum data as a list of floats"""
    
    def __init__(self, pyo_audio_object, fft_size=1024, low_freq=20, high_freq=5000, update_rate=0.05):
        ...
        self.spectrum.setLowFreq(low_freq)
        self.spectrum.setHighFreq(high_freq)
        self.spectrum.polltime(update_rate)
    ...
    def spectrum_callback(self, data):
        ...
        if data and len(data) > 0:
            spectrum_data = data[0]  # First channel
            # Convert to list of floats - assuming each element has a second component
            try:
                self.current_spectrum = [float(x[1]) for x in spectrum_data]
            except (TypeError, IndexError):
                # Fallback if data format is different
                try:
                    self.current_spectrum = [float(x) for x in spectrum_data]
                except (TypeError, ValueError):
                    self.current_spectrum = []
            
            # Calculate frequency bins if not done yet
            if not self.frequency_bins and self.current_spectrum:
    ...
    def get_spectrum(self):
        ...
        return self.current_spectrum.copy() if self.current_spectrum else []
    
    def get_frequency_bins(self):
        """Get the frequency bins"""
        return self.frequency_bins.copy() if self.frequency_bins else []
...
def simple_example():
    """Simple example using the reduced class"""
    # Initialize server
    s = Server().boot()
    s.start()
    
    # Create audio source
    audio_source = (Sine(freq=440, mul=0.3) + 
                   Sine(freq=880, mul=0.2) + 
                   Noise(mul=0.05)).out()
    ...
    # Get spectrum data for a few seconds
    print("Collecting spectrum data...")
    for i in range(50):  # 50 updates
        time.sleep(0.1)
        spectrum = analyzer.get_spectrum()
        if spectrum:
            max_magnitude = max(spectrum)
            max_idx = spectrum.index(max_magnitude)
            freq_bins = analyzer.get_frequency_bins()
            if freq_bins and max_idx < len(freq_bins):
                dominant_freq = freq_bins[max_idx]
                print(f"Frame {i+1}: Dominant frequency = {dominant_freq:.1f} Hz, "
                      f"Magnitude = {max_magnitude:.4f}, Bins = {len(spectrum)}")
...
def example_live_plot():
    ...
    audio_source = (Sine(freq=440, mul=0.3) + 
                   Sine(freq=880, mul=0.2) + 
                   Sine(freq=1760, mul=0.15) +
                   LFO(freq=2, mul=0.1, add=0.1) * Sine(freq=1320) +
                   Noise(mul=0.03)).out()
